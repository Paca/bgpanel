...
/**
 * G2
 * Pour les tests, chemins en durs
 */
require_once("/Users/jeremy/Sites/BGP/libs/phpseclib/SSH2.php");
//define('NET_SSH2_LOGGING', NET_SSH2_LOG_COMPLEX);
/**
 * G2
 * Sans doute inutile on va remplacer les appel ssh2 par sshseclib
 * include('/Users/jeremy/Sites/BGP/libs/phpseclib/SFTP.php');
 */
...
*/
    ...
    protected static function getSftpResource($repoObject){
		...
		*/
		$connection = new Net_SSH2($remote_serv.':'.intval($remote_port));
		if (!$connection->login($remote_user, $remote_pass)) {
			exit('Login Failed');
    	}
		self::$sftpResource = $connection;
    	return self::$sftpResource;
    }
	...
	public static function changeMode($path, $chmodValue){		
		 ...
		 */
		 $connection->exec('chmod '.decoct($chmodValue).' '.$remote_base_path.$url['path']);
	}
	...
	public static function getSshConnection($path, $repoObject = null){
		 ...
		 * ssh2_auth_password($connection, $remote_user, $remote_pass);    
		 */
		$connection = new Net_SSH2($remote_serv.':'.$remote_port);
		if (!$connection->login($remote_user, $remote_pass)) {
			exit('Login Failed');
    	}
